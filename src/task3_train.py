...
def data_preprocess(root):
    train_data = np.zeros((5000, 80*200))
    for i in range(5000):
        ...
        if (img.shape == (80,200)):
            ...
            print('图片大小有问题')
    sio.savemat(r'E:\\文档\\A—重庆大学学校—gpu-killer队—A16项目\\处理后的数据\\data.mat', {'data': train_data}) 
...
def creat_variable(C, IMAGE_HEIGHT, IMAGE_WIDTH):
    ...
    Y = tf.placeholder(tf.float32, [None, C*4], name = 'Y')
...
def forward_propagation(IMAGE_HEIGHT, IMAGE_WIDTH, X, keep_prob,C):
    ...
    with tf.name_scope('output'):
        with tf.name_scope('weights'):
            w_out = tf.get_variable("w_out",[512,C*4], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
            variable_summaries(w_out)
        with tf.name_scope('bias'):
            b_out = tf.get_variable("b_out",[C*4], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def forward_propagation_lenet(w_alpha, b_alpha,IMAGE_HEIGHT, IMAGE_WIDTH, X, keep_prob,C):
    ...
    with tf.name_scope('output'):
        with tf.name_scope('weights'):
            w_out = tf.Variable(w_alpha*tf.random_normal([84,C*4]))
            variable_summaries(w_out)
        with tf.name_scope('bias'):
            b_out = tf.Variable(b_alpha*tf.random_normal([C*4]))
            variable_summaries(b_out)
...
def vec2text(temp):
    ...
    text =  tf.concat([text_1,text_2,text_3,text_4], 1)
    return text
...
def compare(predict, label):
    ...
    correct_pred = tf.equal(temp_1,tf.equal(temp_2,tf.equal(temp_3, temp_4)))
    return correct_pred
...
#        X_ = 255 * np.random.rand(1,IMAGE_HEIGHT*IMAGE_WIDTH)
#        loss_ = output[0,12] + output[0,36+17] + output[0,36*2+25] + output[0,36*3+31] 
#        loss_ += tf.cast(tf.norm(X_),dtype = tf.float32)
#        grad = tf.gradients(loss_, X)[0]
#        
#        for t in range(200):
#            ox, oy = np.random.randint(-16, 16+1, 2)
#            #Xi = X_.copy()
#            X_ = np.roll(np.roll(X_, ox, 0), oy, 1)
#            
#            np_dX = sess.run(grad, feed_dict={X: X_, keep_prob: 1.})
#            X_ += 25 * np_dX
#            
#            X_ = np.roll(np.roll(X_, -ox, 0), -oy, 1)
#            if t % 20 == 0:
#                plt.imshow(X_[0].reshape((IMAGE_HEIGHT,IMAGE_WIDTH)))
...
if __name__ == "__main__":
    #data_preprocess(r'E:\文档\A—重庆大学学校—gpu-killer队—A16项目\处理后的数据\data-3')
    ...
    parameters = model(0.01, 0.1,IMAGE_HEIGHT=80, IMAGE_WIDTH=200, X_train=data, Y_train=label, C=36, keep_prob_value=0.7, minibatch_size=64
