...
def data_preprocess(root,index):
    train_data = np.zeros((66371, 55*80))
    filecount=0
    path = root + "%04d" % index
    ad = root
    for ad,dir,files in  os. walk(path):
        ...
        #print(filecount)
        #print(path)
        #train_data = np.zeros((filecount, 3200))
...
def forward_propagation(IMAGE_HEIGHT, IMAGE_WIDTH, X, keep_prob,C):
    ...
    with tf.name_scope('output'):
        with tf.name_scope('weights'):
            w_out = tf.get_variable("w_out",[512,C], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
            variable_summaries(w_out)
        with tf.name_scope('bias'):
            b_out = tf.get_variable("b_out",[C], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def forward_propagation_lenet(w_alpha, b_alpha,IMAGE_HEIGHT, IMAGE_WIDTH, X, keep_prob,C):
    ...
    with tf.name_scope('output'):
        with tf.name_scope('weights'):
            w_out = tf.Variable(w_alpha*tf.random_normal([84,C]))
            variable_summaries(w_out)
        with tf.name_scope('bias'):
            b_out = tf.Variable(b_alpha*tf.random_normal([C]))
            variable_summaries(b_out)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            for minibatch in minibatches:
                    ...
                    minibatch_Y = convert_to_one_hot(minibatch_Y, C).T
            ...
            if epoch % 10 == 9:
                summary,acc = sess.run([merged,accuracy], feed_dict={X: X_val, Y: Y_val, keep_prob: 1.})
                test_writer.add_summary(summary, epoch)
                print ("Accuracy: %f" % (acc))
                print ("Cost after epoch %i: %f" % (epoch, epoch_cost))
            epoch += 1
            if epoch == 300:
                pat = r'E:\文档\验证码识别\model\task1'
                saver = tf.train.Saver()
                saver.save(sess, pat+"\\model.ckpt")
                print ("save parameters")
                break;
#------------------------------------Saliency Maps------------------------------------------------------------------
#        correct_scores = tf.gather_nd(output,tf.stack((tf.range(6,dtype=tf.int64), tf.argmax(Y, axis=1)),axis=1))
#        grad_img = tf.gradients(correct_scores, X)
#        #Y_s = Y_train[:,0:5].reshape((-1))
#        Y_s = convert_to_one_hot(Y_train[:,0:6], C).T
#        gead_img_val = sess.run(grad_img, feed_dict={X: X_train[0:6,:], Y: Y_s, keep_prob: 1.})[0]
#        
#        #for i in range(6):
#            #plt.subplot(1,6,i+1)
#        img = np.abs(gead_img_val[4].reshape((IMAGE_HEIGHT, IMAGE_WIDTH)))
#        #img = np.clip(img , 0., 255.).astype(np.uint8)
#            #print(img)
#        plt.imshow(img, cmap=plt.cm.hot)
#        plt.axis('off')
            #print(gead_img_val[0].reshape((IMAGE_HEIGHT, IMAGE_WIDTH)))
#---------------------------------------end---------------------------)---------------------------------------------
        train_writer.close()
        test_writer.close()
        print('end')
...
if __name__ == "__main__":
    ...
    #print(count.shape)
    #data_preprocess(r'E:\文档\验证码识别\处理后的数据\data-1' + '\\')
    ...
    parameters = model(0.01, 0.1,IMAGE_HEIGHT=80, IMAGE_WIDTH=55, X_train=data, Y_train=label, C=13, keep_prob_value=0.75, minibatch_size=64
          ,X_val=X_val,Y_val=Y_label)
    #p = predict(0.01, 0.1, X_test,parameters, 80, 55,1, 13)
    #Y_test = convert_to_one_hot(Y_test, 13).T
    #Y_test_t = np.argmax(Y_test, 1)
