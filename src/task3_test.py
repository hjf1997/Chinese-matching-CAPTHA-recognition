...
def forward_propagation(IMAGE_HEIGHT, IMAGE_WIDTH, X, keep_prob,C):
    ...
    with tf.name_scope('output'):
        with tf.name_scope('weights'):
            w_out = tf.get_variable("w_out",[512,C*4])
        with tf.name_scope('bias'):
            b_out = tf.get_variable("b_out",[C*4])
...
def model(IMAGE_HEIGHT, IMAGE_WIDTH, C, keep_prob_value, root, pat):
    ...
    with tf.Session() as sess:
        ...
        for i in range(50000):    #此处输入样例数量，编号从0开始    
            ...
            string += "%04d" % i +',' + transform(p[0,0]) + transform(p[0,1]) + transform(p[0,2]) + transform(p[0,3]) +'\n'
...
if __name__ == "__main__":
    pat = 'E:\\文档\\验证码识别\\model\\task3'#此处输入题目三的模型地址
    root = 'E:\\文档\\验证码识别\\处理后的数据\data-3\\'#此处输入题目三的图片经过matlab处理后的文件夹地址，见文档
    model(IMAGE_HEIGHT=80, IMAGE_WIDTH=200, C=36, keep_prob_value=0.7, root=root, pat=pat)
