# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 16:03:13 2018
...
def data_preprocess(root):
    train_data = np.zeros((10000, 150*45))
...
def creat_variable(C, IMAGE_HEIGHT, IMAGE_WIDTH):
    ...
    Y = tf.placeholder(tf.float32, [None, C], name = 'Y')
...
def model(w_alpha, b_alpha,IMAGE_HEIGHT, IMAGE_WIDTH, X_train, Y_train, C, 
    ...
    with tf.name_scope('train'):
        ...
        earning_rate = tf.train.exponential_decay(initial_learning_rate,
                                                  ...
                                                  decay_steps=60,decay_rate=0.8)
        tf.summary.scalar('learning_rate', earning_rate)
        optimizer = tf.train.AdamOptimizer(learning_rate=earning_rate).minimize(loss)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            summary, _, _ = sess.run([merged, optimizer, loss], feed_dict={X: minibatch_X, Y: minibatch_Y, keep_prob: keep_prob_value})
...
#    data_preprocess(r'E:\文档\验证码识别\处理后的数据\data-4_raw')
    ...
    parameters = model(0.01, 0.1,IMAGE_HEIGHT=45, IMAGE_WIDTH=150, X_train=data, Y_train=label, C=4, keep_prob_value=0.7, minibatch_size=64
